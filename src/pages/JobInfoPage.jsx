    ...
    <div className="bg-background min-h-screen text-secondary">
      ...
      <main className="flex flex-wrap md:flex-nowrap">
        <aside className="px-4 mb-4 md:px-8 w-full h-fit md:max-w-xs bg-background ">
            ...
            <div>
                <h3 className="text-[#B9BDCF] uppercase text-base text-bold mb-4">How to Apply</h3>
                <div className="mb-1">
                Company website : { state.page.employer_website != null ? <a href={state.page.employer_website} target="_blank" rel="noopener noreferrer" className="text-primary">{state.page.employer_name}</a> : <a href={state.page.employer_website} target="_blank" rel="noopener noreferrer" className="text-gray-500 cursor-not-allowed ">{state.page.employer_name}</a>}
                </div>
                <div className="mb-1">
                    Salary : As per the Industry Standard
                </div>
                <div className="mb-1">
                    Publisher : {state.page.job_publisher ? state.page.job_publisher : "NA" }
                </div>
                <div className="mb-4 ">
                    Apply Before : <span className="underline underline-offset-2 decoration-wavy  decoration-violet-500">{dateTimeExp.toDateString()}</span>
                </div>
                <a href={state.page.job_apply_link} target="_blank" rel="noopener noreferrer" >
                    <button className="bg-primary mb-4 text-white px-4 py-2 text-base rounded-md flex-none sm:px-4 sm:py-2  hover:bg-sky-700"> Apply Now</button>
                </a>
            </div>
        </aside>
        <section className="w-full md:w-3/4 px-4 md:px-8">
            ...
            <div className="text-gray-400 truncate text-xs justify-self-end  mb-8 gap-2 flex items-center">
                <BiTimeFive className="inline"/><span className=""> {dateTimePosted.toDateString()}</span>
            </div>
            ...
            <div className="flex gap-4 flex-wrap mb-8">
                ...
                <div className="flex flex-col justify-around">
                    ...
                    <div className="text-gray-400 text-xs truncate flex items-center gap-2">
                        <BiWorld className="inline"/><span className="">{state.page.job_country}</span>
                    </div>
                </div>
            </div>
            <div >
                <div className="mb-8">
                <h3 className="text-base font-bold truncate mb-4">🚀 Job Info: </h3>
                <hr className="mb-4 border-secondary"></hr>
                    <div className="font-Poppins whitespace-break-spaces" >{parse(des)}</div>
                </div>
                {
                    keys.map((item, i)=>{
                        return(
                            <div key={i} className="mb-8">
                                <h3 className="text-base font-bold truncate mb-4">{"🚀  "+item+":"}</h3>
                                <hr className="mb-4 border-secondary"></hr>
                                <ul className="list-disc ml-4">
                                    {state.page.job_highlights[item].map((pro,j) => <li key={j} className="mb-2"><span>{pro}</span></li>)}
                                </ul>
                            </div>
                        )
                    })
